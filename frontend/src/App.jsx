...
function ModernAttendanceCard() {
  ...
  return <>
    <section className={`card attendance-card attendance-state-${state.toLowerCase()}`}>
      ...
      {pendingApproval && <div className="late-policy-banner"><strong>Manual {attendance.manualRequest.action?.replace('_','-')} awaiting approval</strong><span>An authorized reviewer will assess the reason, server request time, and available attendance evidence.</span></div>}
