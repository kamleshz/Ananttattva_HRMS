...
function ModernAttendanceCard() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setSelectedMode(today.attendanceMode || arrangements.modes?.find(item=>item!=="office") || "office");
    }
    catch (requestError) { setError(requestError.message); }
    finally { setLoading(false); }
  };
  useEffect(() => {
    Promise.all([attendanceApi.today(),workArrangementApi.today()]).then(([today,arrangements])=>{setAttendance(today);setAvailableModes(arrangements.modes||["office"]);setSelectedMode(today.attendanceMode||arrangements.modes?.find(item=>item!=="office")||"office")}).catch(requestError=>setError(requestError.message)).finally(()=>setLoading(false));
  }, []);
  useEffect(() => {
  ...
  return <>
    <section className={`card attendance-card attendance-state-${state.toLowerCase()}`}>
      <div className="card-heading"><div><p className="eyebrow">Today&apos;s attendance</p><h2>{shift.name}</h2></div><span className={`status ${state === "NOT_CHECKED_IN" ? "neutral" : "present"}`}><i/>{completed ? "Day completed" : pendingApproval ? "Approval pending" : checkedIn ? "Checked in" : "Not checked in"}</span></div>
      ...
      {pendingApproval && <div className="manual-pending-card"><div><span className="pending-pill"><i/>Pending Approval</span><strong>Manual {attendance.manualRequest.action==='check_out'?'Check-Out':'Check-In'} Pending</strong><small>Submitted at {new Date(attendance.manualRequest.requestedAt).toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}</small></div><button onClick={()=>navigate('/attendance')}>View Request <ChevronRight size={14}/></button></div>}
      ...
      {!pendingApproval && !completed && !error && <><button className={`attendance-button ${checkedIn ? "checkout" : ""}`} onClick={() => setDrawerOpen(true)}>{checkedIn ? "Check Out with Face" : `Check In with Face · ${selectedMode==='office'?'Office':selectedMode.replaceAll('_',' ')}`}<ChevronRight size={17}/></button><button className="manual-fallback-link" onClick={()=>setManualFallback({})}><Camera size={13}/><span>Having trouble? Use manual {checkedIn?'check-out':'check-in'}</span><ChevronRight className="manual-link-arrow" size={13}/></button></>}
      {completed && <button className="full-link" onClick={() => navigate("/attendance")}>View attendance details <ChevronRight size={15}/></button>}
      <div className="requirements"><span><CheckCircle2 size={14}/>Location optional</span><span><CheckCircle2 size={14}/>Photo and face verification required</span></div>
    </section>
...
function HomePage({ user, dashboard }) {
  ...
  return (
    <div className="home-dashboard">
      <section className="dashboard-welcome-panel dashboard-greeting-only">
...
export default function App() {
  ...
  const pages = {
    ...
    "/leave": <LeavePage user={user} currentEmployeeId={dashboard?.employee?._id || user.employee?._id || user.employee} />,
    "/requests": <RequestsPage user={user} currentEmployeeId={dashboard?.employee?._id || user.employee?._id || user.employee} onPendingCountChange={setPendingRequestCount} />,
