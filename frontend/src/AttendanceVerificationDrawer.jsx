...
import { attendanceApi, biometricApi, organizationApi, workArrangementApi } from './services/api.js'
...
export default function AttendanceVerificationDrawer({ mode, attendanceMode='office', close, recorded }) {
  ...
  useEffect(()=>{
    ...
    const eligibility=attendanceMode==='office'?organizationApi.officeLocations():workArrangementApi.today().then(result=>{
      if(!result.modes.includes(attendanceMode))throw new Error(`An approved ${attendanceMode.replaceAll('_',' ')} request is required for today.`)
      return [{}]
    })
    eligibility.then(locations=>{
      if(!active)return
