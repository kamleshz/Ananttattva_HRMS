...
import { attendanceApi, biometricApi } from './services/api.js'
import { challengeCopy, createIdentityTemplate, detectFace, evaluateChallenge, loadFaceIdentityModel, loadFaceLandmarker } from './services/biometrics.js'
...
export default function AttendanceVerificationDrawer({ mode, close, recorded }) {
  ...
  useEffect(()=>{
    ...
    let frame,active=true,lastProcessed=0,neutralFrames=0,passedFrames=0
    const process=(time)=>{
      if(!active)return
      if(time-lastProcessed>90&&videoRef.current?.readyState>=2){
        ...
        try{
          ...
          if(evaluation.faceCount===0)setLivenessStatus('Position one face inside the frame.')
            ...
            if(passedFrames>=(challenge.challenge==='blink'?1:2)){
              ...
              setPhoto(canvas.toDataURL('image/jpeg',.82));setLivenessScore(Math.max(.66,Math.min(1,.68+evaluation.score*.32)));setLivenessStatus('Liveness passed. Preparing identity match…');stream.getTracks().forEach(track=>track.stop());return
            }
          }
  ...
  async function submit(){if(!photo||!coordinates||!faceTemplate||!challenge)return;setBusy(true);setError('');try{const verification=await biometricApi.verify({challengeToken:challenge.challengeToken,challenge:challenge.challenge,photo,faceTemplate,livenessScore,faceCount:1});const payload={photo,attendanceMode:'office',locationVerified:true,location:coordinates,biometricToken:verification.verificationToken};const result=mode==='check-out'?await attendanceApi.checkOut(payload):await attendanceApi.checkIn(payload);recorded(result);close()}catch(requestError){setError(requestError.message)}finally{setBusy(false)}}
  async function retake(){stream?.getTracks().forEach(track=>track.stop());setPhoto('');setFaceTemplate(null);setChallenge(null);setLivenessScore(0);setError('');setCameraError('');setLivenessStatus('Preparing a new challenge…');try{await Promise.all([openCamera(),requestChallenge()])}catch{setCameraError('Unable to restart biometric verification.')}}
  const ready=Boolean(photo&&coordinates&&faceTemplate?.length>=128&&livenessScore>=.65),challengeText=challengeCopy[challenge?.challenge]
  return <div className="drawer-layer"><button className="drawer-backdrop" onClick={close}/><aside className="attendance-drawer"><div className="drawer-heading"><div><p className="eyebrow">Secure attendance verification</p><h2>{mode==='check-out'?'Web check out':'Web check in'}</h2><p>Live face, identity and location verification are required.</p></div><button onClick={close}><X size={20}/></button></div><div className="verification-section"><p className="verification-label">1 · Current location</p>{coordinates?<div className="verification-success"><CheckCircle2 size={18}/><div><strong>Location captured</strong><span>{coordinates.latitude.toFixed(6)}, {coordinates.longitude.toFixed(6)}</span></div></div>:locationError?<div className="verification-error">{locationError}</div>:<div className="verification-loading"><span/> Getting your precise location…</div>}</div><div className="verification-section"><p className="verification-label">2 · Active liveness and identity check</p>{challengeText&&<div className="liveness-challenge"><span><ScanFace size={21}/></span><div><small>Random challenge</small><strong>{challengeText.title}</strong><p>{challengeText.instruction}</p></div></div>}<div className={`camera-frame ${faceTemplate?'liveness-passed':''}`}>{photo?<img src={photo} alt="Attendance identity capture"/>:<video ref={videoRef} muted playsInline/>}{!photo&&!stream&&!cameraError&&<div className="camera-placeholder"><UserRound size={30}/><span>Opening secure camera…</span></div>}{cameraError&&<div className="camera-placeholder error"><UserRound size={30}/><span>{cameraError}</span></div>}<div className="face-guide"/>{photo&&<div className="liveness-overlay"><ShieldCheck size={25}/><strong>{faceTemplate?'Identity ready':'Analyzing identity…'}</strong><span>{faceTemplate?'Server match is required before attendance':'Creating secure face embedding'}</span></div>}</div><canvas ref={canvasRef} hidden/><div className={`liveness-status ${faceTemplate?'passed':''}`}>{faceTemplate?<CheckCircle2 size={16}/>:<ScanFace size={16}/>}<span>{livenessStatus}</span></div>{photo&&<button className="secondary-button camera-action" onClick={retake}>Repeat verification</button>}</div>{error&&<p className="attendance-error drawer-error">{error}</p>}<div className="attendance-drawer-footer"><div><Clock3 size={16}/><span>Attendance is blocked unless this face matches the employee’s enrolled identity.</span></div><button className="primary-button" disabled={!ready||busy} onClick={submit}>{busy?'Matching identity and recording…':mode==='check-out'?'Confirm verified check out':'Confirm verified check in'} <ChevronRight size={15}/></button></div></aside></div>
}
