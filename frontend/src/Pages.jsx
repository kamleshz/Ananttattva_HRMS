  ...
  return <div className="mis-dashboard">
    ...
    {!data ? <StateMessage>Loading attendance MIS…</StateMessage> : <>
      ...
      <section className="content-card mis-table-card"><div className="mis-table-heading"><div><span>{data.label}</span><h2>Employee attendance performance</h2></div><p>{(() => {
        ...
        return <>Targets: <strong>Full day {fullH}h {String(fullM).padStart(2, "0")}m</strong> · <strong>Half day (applied leave only) {halfH}h {String(halfM_f).padStart(2, "0")}m</strong> · <em style={{color:'#0f766e'}}>Total Working Hours: per calendar working days (Sun/1st&amp;3rd Sat/public holidays excluded) minus approved leaves. Completed Working Hours: full-leave days skipped; applied half-day capped @ half-day target; normal days capped @ full-day target.</em></>;
      })()}</p></div><div className="data-table-wrap"><table className="data-table mis-table"><thead><tr><th rowSpan="2">Employee</th><th rowSpan="2">Employee ID</th><th rowSpan="2">Department</th><th className="mis-group-heading" colSpan="2">Late Arrival</th><th className="mis-group-heading" colSpan="3">Completed Days</th><th rowSpan="2">Total Working Hours</th><th rowSpan="2">Completed Working Hours</th><th rowSpan="2">Leave before 8:30</th><th rowSpan="2">Leave on 8:30</th><th rowSpan="2">Leave after 8:30</th></tr><tr className="mis-subhead"><th>10:15</th><th>10:30</th><th>Full day</th><th>Half day (applied leave)</th><th>Incomplete half (late 3-day rule)</th></tr></thead><tbody>{data.rows.length === 0 ? <tr><td colSpan="14" className="mis-empty-row">No attendance records for this date range.</td></tr> : data.rows.map(row=><tr key={row.employeeId}><td><strong>{row.name}</strong><small>{row.designation}</small></td><td>{row.employeeCode}</td><td>{row.department}</td><td><b className="mis-count late">{row.lateAt1015}</b></td><td><b className="mis-count late">{row.lateAt1030}</b></td><td>{row.fullDays}</td><td>{row.halfDays}</td><td><b className="mis-count less">{row.incompleteHalfDays}</b></td><td><strong>{hours(row.expectedWorkingMinutes)}</strong></td><td><strong style={{color:'#0f766e'}}>{hours(row.completedWorkingMinutes||0)}</strong></td><td><b className="mis-count less">{row.lessThanTarget}</b></td><td><b className="mis-count equal">{row.equalToTarget}</b></td><td><b className="mis-count more">{row.moreThanTarget}</b></td></tr>)}</tbody></table></div></section>
    </>}
  </div>;
}
