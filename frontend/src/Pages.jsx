...
export function EmployeeBiometricPage({ employeeId }) {
  ...
  async function runAdminAction(action) {
    ...
    try {
      if (action === "reset") {
        if (!window.confirm("Reset this employee's SFace access? Legacy biometric data will be preserved.")) return;
  ...
  return <>
    ...
    <section className="content-card biometric-step-card">
      {SERVER_FACE_ENABLED && status && <div className="existing-biometric-photos"><div><strong>SFace biometric status</strong><span>{status.enrolled ? "Encrypted server-side identity is active." : "Server-side identity is not active."}</span></div><div className="biometric-status-grid"><span><b>Migration</b>{status.migrationStatus.replaceAll("_", " ")}</span><span><b>Engine</b>{status.engine || "Not enrolled"}</span><span><b>Model</b>{status.modelVersion || "Not enrolled"}</span><span><b>Embedding</b>{status.embeddingDimension ? `${status.embeddingDimension} dimensions` : "Unavailable"}</span><span><b>Enrolled</b>{status.enrolledAt ? new Date(status.enrolledAt).toLocaleString() : "Not enrolled"}</span><span><b>Last verification</b>{status.lastVerificationAt ? new Date(status.lastVerificationAt).toLocaleString() : "Never"}</span><span><b>Last result</b>{status.lastVerificationResult?.replaceAll("_", " ") || "Unavailable"}</span><span><b>Similarity</b>{status.lastSimilarityScore ?? "Unavailable"}</span><span><b>Liveness</b>{status.lastLivenessScore ?? "Unavailable"}</span><span><b>Failure reason</b>{status.lastFailureReason?.replaceAll("_", " ") || "None"}</span><span><b>30-day exceptions</b>{status.recentVerificationFailureCount} failed · {status.manualAttendanceCount} manual</span></div><div className="onboarding-actions">{status.migrationStatus === "migration_required" && <button type="button" className="secondary-button" disabled={busy} onClick={() => runAdminAction("migrate")}>Migrate trusted photos</button>}{status.enrolled && <button type="button" className="secondary-button" disabled={busy} onClick={() => runAdminAction("reset")}>Reset face access</button>}</div></div>}
    ...
    <>
      ...
      {error ? (
        ...
        <>
          ...
          {biometricReport && <section className="content-card report-chart"><div className="section-heading"><div><p className="eyebrow">Last 30 days</p><h2>Biometric health</h2></div></div><div className="report-metrics">{[["Success rate", `${biometricReport.metrics.biometricSuccessRate}%`],["Face match failures", `${biometricReport.metrics.faceMatchFailureRate}%`],["Liveness failures", `${biometricReport.metrics.livenessFailureRate}%`],["Camera failures", `${biometricReport.metrics.cameraFailureRate}%`],["Manual fallback", `${biometricReport.metrics.manualFallbackRate}%`],["Re-enrollment required", biometricReport.metrics.employeesRequiringReEnrollment],["Old templates", biometricReport.metrics.employeesUsingOldTemplate],["Migrated to SFace", biometricReport.metrics.employeesMigratedToSFace]].map(([label,value])=><div key={label}><span><ScanFace size={18}/></span><p>{label}</p><strong>{value}</strong></div>)}</div></section>}
          {biometricReport?.employees?.length > 0 && <section className="content-card"><div className="section-heading"><div><p className="eyebrow">Employee security</p><h2>Biometric enrollment status</h2></div></div><div className="table-shell"><table className="data-table"><thead><tr><th>Employee</th><th>Status</th><th>Engine / model</th><th>Migration</th><th>Last verification</th><th>Similarity</th><th>Liveness</th><th>Failure</th></tr></thead><tbody>{biometricReport.employees.map(employee=><tr key={employee.employeeId}><td><strong>{employee.employeeName}</strong><small>{employee.employeeCode}</small></td><td>{employee.enrollmentStatus?.replaceAll('_',' ')}</td><td>{employee.engine || 'Legacy / none'}<small>{employee.modelVersion || ''}</small></td><td>{employee.migrationStatus?.replaceAll('_',' ')}</td><td>{employee.lastVerificationAt ? new Date(employee.lastVerificationAt).toLocaleString() : 'Never'}<small>{employee.lastVerificationResult?.replaceAll('_',' ') || ''}</small></td><td>{employee.similarityScore ?? '—'}</td><td>{employee.livenessScore ?? '—'}</td><td>{employee.failureReason?.replaceAll('_',' ') || '—'}</td></tr>)}</tbody></table></div></section>}
        </>
      )}
    </>
  );
  ...
  return <div className="mis-dashboard">
    ...
    {!data ? <StateMessage>Loading attendance MIS…</StateMessage> : <>
      ...
      <section className="content-card mis-table-card"><div className="mis-table-heading"><div><span>{data.label}</span><h2>Employee attendance performance</h2></div><p>{(() => {
        ...
        return <>Targets: <strong>Full day {fullH}h {String(fullM).padStart(2, "0")}m</strong> · <strong>Half day (applied leave only) {halfH}h {String(halfM_f).padStart(2, "0")}m</strong> · <em style={{color:'#0f766e'}}>Total Working Hours: per calendar working days (Sun/1st&amp;3rd Sat/public holidays excluded) minus approved leaves.  Completed Working Hours (target-capped, no overtime): full-leave skipped; half/early-leave capped @ reduced target; normal days capped @ full target.  Actual Worked Hours (Excel match): raw CheckIn → CheckOut sum, overtime included — same value as attendance export "Working Hours" column.</em></>;
      })()}</p></div><div className="data-table-wrap"><table className="data-table mis-table"><thead><tr><th rowSpan="2">Employee</th><th rowSpan="2">Employee ID</th><th rowSpan="2">Department</th><th className="mis-group-heading" colSpan="2">Late Arrival</th><th className="mis-group-heading" colSpan="3">Completed Days</th><th rowSpan="2">Total Working Hours</th><th rowSpan="2">Completed Working Hours</th><th rowSpan="2">Actual Worked Hours</th><th rowSpan="2">Leave before 8:30</th><th rowSpan="2">Leave on 8:30</th><th rowSpan="2">Leave after 8:30</th></tr><tr className="mis-subhead"><th>10:15</th><th>10:30</th><th>Full day</th><th>Half day (applied leave)</th><th>Incomplete half (late 3-day rule)</th></tr></thead><tbody>{data.rows.length === 0 ? <tr><td colSpan="15" className="mis-empty-row">No attendance records for this date range.</td></tr> : data.rows.map(row=><tr key={row.employeeId}><td><strong>{row.name}</strong><small>{row.designation}</small></td><td>{row.employeeCode}</td><td>{row.department}</td><td><b className="mis-count late">{row.lateAt1015}</b></td><td><b className="mis-count late">{row.lateAt1030}</b></td><td>{row.fullDays}</td><td>{row.halfDays}</td><td><b className="mis-count less">{row.incompleteHalfDays}</b></td><td><strong>{hours(row.expectedWorkingMinutes)}</strong></td><td><strong style={{color:'#0f766e'}}>{hours(row.completedWorkingMinutes||0)}</strong></td><td><strong style={{color:'#b45309'}}>{hours(row.actualWorkedMinutes||0)}</strong></td><td><b className="mis-count less">{row.lessThanTarget}</b></td><td><b className="mis-count equal">{row.equalToTarget}</b></td><td><b className="mis-count more">{row.moreThanTarget}</b></td></tr>)}</tbody></table></div></section>
    </>}
  </div>;
}
