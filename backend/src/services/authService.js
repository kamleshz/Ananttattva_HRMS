...
import { sendLoginOtp } from './mailService.js'
...
export async function login(email, password, loginType = 'user') {
  ...
  const recentChallenge = await LoginOtp.findOne({ user: user._id, createdAt: { $gt: new Date(Date.now() - 60_000) }, usedAt: null })
  ...
  const challenge = await LoginOtp.create({ challengeId, user: user._id, codeHash: hashCode(challengeId, code), expiresAt })
  try {
...
export async function verifyLoginOtp(challengeId, code) {
  const challenge = await LoginOtp.findOne({ challengeId }).select('+codeHash')
