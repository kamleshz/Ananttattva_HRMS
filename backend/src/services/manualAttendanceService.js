...
export async function createManualAttendanceRequest({req,input,mismatchProof,proofPhotoSource}){
  ...
  if(input.reasonCode==='LIVE_CAMERA_VERIFICATION'&&!proofPhotoSource&&!input.proofPhoto)throw new HttpError(422,'A live verification photo is required')
  ...
  const proofPhoto=await uploadAttendanceProof(proofPhotoSource||input.proofPhoto,employee._id,input.action)
