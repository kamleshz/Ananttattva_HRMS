...
class BiometricService:
    def __init__(
        self, database: AsyncDatabase[dict[str, Any]], redis: Redis, settings: Settings, manager: FaceEngineManager
    ) -> None:
    ...
    async def _target_employee(self, user: dict[str, Any], requested: str | None) -> str:
        ...
        if (
            ...
            or not await self.database.employees.find_one({"_id": ObjectId(target)}, {"_id": 1})
        ):
    ...
    async def create_challenge(self, user: dict[str, Any], payload: ChallengeRequest) -> ChallengeResult:
        ...
        expires_in = (
            self.settings.face_enrollment_challenge_expiry_seconds
            if payload.action == "enroll"
            else self.settings.face_challenge_expiry_seconds
        )
        now = datetime.now(UTC)
        document = {
            "challengeId": challenge_id,
            "userId": _identifier(user["_id"]),
            "employeeId": employee_id,
            "action": payload.action,
            "attendanceMode": payload.attendance_mode,
            "steps": steps,
        ...
        return ChallengeResult(
            challenge_id=challenge_id, steps=steps, expires_in=expires_in, max_retries=self.settings.face_max_retries
        )

    async def _consume_challenge(
        self, challenge_id: str, user: dict[str, Any], completed_steps: list[str], expected_action: str | None = None
    ) -> dict[str, Any]:
        ...
        if challenge.get("userId") != _identifier(user["_id"]):
            raise AppError(
                403, "This liveness challenge belongs to another user", [{"code": "CHALLENGE_EMPLOYEE_MISMATCH"}]
            )
    ...
    async def _analyze(self, photo: str, *, require_liveness: bool = False) -> FaceAnalysis:
        return await asyncio.to_thread(self.manager.get().analyze, photo, require_liveness=require_liveness)

    @staticmethod
    def _aggregate(analyses: list[FaceAnalysis]) -> Any:
        vectors = [item.embedding for item in analyses]
        mean = sum(vectors) / len(vectors)
        return mean / math.sqrt(float((mean * mean).sum()))

    async def enroll(
        self, user: dict[str, Any], payload: EnrollmentRequest, request_meta: dict[str, str | None]
    ) -> BiometricStatus:
        ...
        similarities = [
            engine.compare_embeddings(analyses[i].embedding, analyses[j].embedding)
            for i in range(3)
        ...
        if min(similarities) < self.settings.face_match_threshold:
            raise AppError(
                422,
                "Enrollment photos are not sufficiently consistent. Retake all three poses.",
                [{"code": "ENROLLMENT_IDENTITY_INCONSISTENT"}],
            )
        old = await self.repository.employee(payload.employee_id)
        await self.repository.save_template(
            payload.employee_id,
            embeddings=[self._aggregate(analyses)],
            metadata={
                "engineName": self.settings.face_engine,
                "detectorName": self.settings.face_detector,
                "recognizerName": self.settings.face_recognizer,
                "modelVersion": self.settings.face_model_version,
                ...
                "enrollmentQuality": [
                    {
                        ...
                        "photoHash": hashlib.sha256(sample.photo.encode()).hexdigest(),
                    }
                    for sample, analysis in zip(payload.samples, analyses, strict=True)
                ],
            },
        )
        action = (
            "BIOMETRIC_REENROLLED"
            if old and (old.get("faceBiometric") or old.get("biometricTemplateVersion"))
            else "BIOMETRIC_ENROLLED"
        )
        await self.audit.record(
            action=action,
            entity_type="Employee",
            entity_id=payload.employee_id,
            actor_user_id=_identifier(user["_id"]),
            actor_employee_id=_identifier(user.get("employee")) or None,
            role=user.get("role"),
            metadata={
                "engine": self.settings.face_engine,
                "modelVersion": self.settings.face_model_version,
                "sampleCount": 3,
            },
            **request_meta,
    ...
    async def _assess_location(self, employee_id: str, mode: str, location: Any | None) -> dict[str, Any]:
        def distance(target: dict[str, Any]) -> float:
            lat1, lon1, lat2, lon2 = map(
                math.radians, [location.latitude, location.longitude, target["latitude"], target["longitude"]]
            )
            value = (
        ...
        if mode == "office":
            ...
            if not offices:
                return {
                    ...
                    "accuracyMeters": round(location.accuracy_meters),
                }
            ...
            return {
                "verified": accurate and within_boundary,
                "status": "verified"
                if accurate and within_boundary
                else "low_accuracy"
                if not accurate
                else "outside_boundary",
                "officeId": str(office["_id"]),
                "latitude": location.latitude,
                "longitude": location.longitude,
                "distanceMeters": round(measured),
                "accuracyMeters": round(location.accuracy_meters),
            }
        now = datetime.now(UTC)
        arrangement = await self.database.workarrangementrequests.find_one(
            {
        ...
        if not arrangement:
            raise AppError(
                403, "An approved work arrangement is required for today", [{"code": "WORK_ARRANGEMENT_REQUIRED"}]
            )
        if location is None:
        ...
        if location.accuracy_meters > 150:
            return {
                "verified": False,
                "status": "low_accuracy",
                "arrangementId": str(arrangement["_id"]),
                "latitude": location.latitude,
                "longitude": location.longitude,
                "accuracyMeters": round(location.accuracy_meters),
            }
        ...
        return {
            "verified": within_boundary,
            "status": "verified" if within_boundary else "outside_boundary",
            "arrangementId": str(arrangement["_id"]),
            "latitude": location.latitude,
            "longitude": location.longitude,
            "distanceMeters": round(measured),
    ...
    async def verify(
        self, user: dict[str, Any], payload: VerificationRequest, request_meta: dict[str, str | None]
    ) -> VerificationResult:
        ...
        if not templates:
            ...
            code = (
                ...
                if status["migrationStatus"] != "not_enrolled"
                else "BIOMETRIC_NOT_ENROLLED"
            )
            raise AppError(409, "SFace enrollment is required before face attendance can be used", [{"code": code}])
        try:
            analysis = await self._analyze(payload.proof_image, require_liveness=True)
            location = await self._assess_location(
                challenge["employeeId"], challenge["attendanceMode"], payload.location
            )
            engine = self.manager.get()
            ...
            now = datetime.now(UTC)
            common = {
                "sub": _identifier(user["_id"]),
                "employeeId": challenge["employeeId"],
                "mode": challenge["action"],
                "attendanceMode": challenge["attendanceMode"],
                "challengeId": payload.challenge_id,
                "photoHash": photo_hash,
                "proofRef": photo_hash,
                "livenessScore": round(analysis.anti_spoof_confidence or 0.0, 6),
                "faceMatchScore": round(similarity, 6),
                "identityTemplateVersion": 5,
                "engineName": self.settings.face_engine,
                "modelVersion": self.settings.face_model_version,
                "locationVerified": bool(location.get("verified")),
            ...
            if similarity < self.settings.face_match_threshold:
                result_code = (
                    ...
                    if similarity >= self.settings.face_match_threshold - self.settings.face_low_confidence_margin
                    else "FACE_MATCH_FAILED"
                )
                mismatch = jwt.encode(
                    {
                        **common,
                        "purpose": "biometric_mismatch",
                ...
                await self.audit.record(
                    action=result_code,
                    entity_type="Employee",
                    entity_id=challenge["employeeId"],
                    actor_user_id=_identifier(user["_id"]),
                    actor_employee_id=challenge["employeeId"],
                    role=user.get("role"),
                    metadata={
                        "engine": self.settings.face_engine,
                        "modelVersion": self.settings.face_model_version,
                        ...
                        "livenessScore": common["livenessScore"],
                        "attendanceMode": challenge["attendanceMode"],
                        "locationVerified": common["locationVerified"],
                    },
                    **request_meta,
                )
                return VerificationResult(
                    verified=False, message="We couldn't verify your face. Please try again.", mismatch_token=mismatch
                )
            jti = str(uuid.uuid4())
            token = jwt.encode(
                {
                    **common,
                    ...
                    "verified": True,
                    "jti": jti,
                    "exp": now + timedelta(seconds=self.settings.face_verification_expiry_seconds),
                },
                self.settings.jwt_secret.get_secret_value(),
                algorithm="HS256",
            )
            await self.redis.set(
                f"face:verification:{jti}",
                json.dumps({"state": "pending", "employeeId": challenge["employeeId"], "action": challenge["action"]}),
                ex=self.settings.face_verification_expiry_seconds,
            )
            await self.audit.record(
                ...
                metadata={
                    ...
                    "livenessPassed": True,
                    "livenessScore": common["livenessScore"],
                    "antiSpoofPassed": analysis.anti_spoof_passed,
                    "attendanceMode": challenge["attendanceMode"],
                    "locationEvidence": location,
                },
                **request_meta,
            ...
            return VerificationResult(
                ...
                expires_in=self.settings.face_verification_expiry_seconds,
            )
            ...
            await self.audit.record(
                ...
                metadata={
                    "errorCode": (
                        exc.details[0].get("code")
                        if exc.details and isinstance(exc.details[0], dict)
                        else "UNKNOWN_ERROR"
                    ),
                    "engine": self.settings.face_engine,
                    "modelVersion": self.settings.face_model_version,
    ...
    async def migrate(
        self, user: dict[str, Any], employee_id: str, request_meta: dict[str, str | None]
    ) -> MigrationResult:
        ...
        if employee.get("faceBiometric", {}).get("encryptedTemplates"):
            ...
            if (
                ...
                return MigrationResult(
                    ...
                    message="The employee already has a compatible SFace template.",
                )
        ...
        if len(trusted) != 3:
            ...
            result = MigrationResult(
                ...
                message="Trusted enrollment photos are unavailable; live re-enrollment is required.",
            )
        else:
            ...
            await self.repository.save_template(
                ...
                metadata={
                    ...
                    "enrollmentQuality": [
                        ...
                        for sample, analysis in zip(trusted, analyses, strict=True)
                    ],
                },
            )
            result = MigrationResult(
                employee_id=employee_id,
                status="migrated",
                message="Existing trusted photos were migrated to SFace. Legacy data was preserved for rollback.",
            )
        await self.audit.record(
            action="BIOMETRIC_MIGRATED" if result.status == "migrated" else "BIOMETRIC_REENROLLMENT_REQUIRED",
            entity_type="Employee",
            entity_id=employee_id,
            actor_user_id=_identifier(user["_id"]),
            actor_employee_id=_identifier(user.get("employee")) or None,
            role=user.get("role"),
            metadata={"status": result.status, "legacyDataPreserved": True},
            **request_meta,
        )
        return result

    async def migrate_batch(
        self, user: dict[str, Any], payload: BatchMigrationRequest, request_meta: dict[str, str | None]
    ) -> list[MigrationResult]:
        ids = payload.employee_ids
        if not ids:
            cursor = self.database.employees.find(
                {"faceBiometric.encryptedTemplates": {"$exists": False}, "biometricTemplate": {"$exists": True}},
                {"_id": 1},
            ).limit(100)
        ...
        if payload.dry_run:
            ...
            for employee_id in ids:
                ...
                results.append(
                    MigrationResult(employee_id=employee_id, status=status, message="Dry run only; no data changed.")
                )
            return results
    ...
    async def health_report(self, days: int) -> dict[str, Any]:
        ...
        events = (
            await self.database.auditLogs.find(
        ...
        employees = await self.database.employees.find(
            {},
            {
                "employeeCode": 1,
                "firstName": 1,
        ...
        liveness_failures = sum(
            item["action"] in {"LIVENESS_FAILED", "ANTI_SPOOF_FAILED"}
            or item.get("metadata", {}).get("errorCode") in {"LIVENESS_FAILED", "ANTI_SPOOF_FAILED"}
            for item in events
        )
        camera_codes = {
        ...
        camera_failures = await self.database.faceattendancerequests.count_documents(
            {
                "requestedAt": {"$gte": since},
                "$or": [
                    {"reasonCode": {"$in": list(camera_codes)}},
                    {"biometricAttempt.technicalErrorCode": {"$in": list(camera_codes)}},
                ],
            }
        )
        ...
        total_attempts = successes + len(
            [item for item in events if item["action"] != "BIOMETRIC_VERIFICATION_SUCCESS"]
        )
        migrated = sum(item.get("faceBiometric", {}).get("migrationStatus") == "migrated" for item in employees)
        requiring = sum(
            item.get("faceBiometric", {}).get("enrollmentRequired", False)
            or item.get("faceBiometric", {}).get("migrationStatus") == "re_enrollment_required"
            for item in employees
        )
        old_templates = sum(
            bool(item.get("biometricTemplate")) and not item.get("faceBiometric", {}).get("encryptedTemplates")
            for item in employees
        )
        latest_by_employee: dict[str, dict[str, Any]] = {}
        for event in events:
            latest_by_employee.setdefault(str(event.get("entityId")), event)
        ...
        for employee in employees:
            ...
            if encrypted and compatible:
                migration_status = biometric.get("migrationStatus", "compatible")
            elif encrypted or employee.get("biometricTemplate"):
                migration_status = "re_enrollment_required"
            else:
                migration_status = "not_enrolled"
            latest = latest_by_employee.get(employee_id) or {}
            ...
            employee_statuses.append(
                {
                    ...
                    if migration_status == "re_enrollment_required"
                    else "not_enrolled",
                    "engine": biometric.get("engineName"),
                    "modelVersion": biometric.get("modelVersion"),
                    "enrolledAt": biometric.get("enrolledAt"),
                    "migrationStatus": migration_status,
                    "reEnrollmentRequired": migration_status == "re_enrollment_required"
                    or bool(biometric.get("enrollmentRequired")),
                    "lastVerificationAt": latest.get("timestamp"),
                    "lastVerificationResult": latest.get("action"),
                    "similarityScore": latest_meta.get("similarity"),
                    "livenessScore": latest_meta.get("livenessScore"),
                    "failureReason": latest_meta.get("errorCode"),
                }
            )

        counters: dict[str, Counter[str]] = {
            name: Counter() for name in ("browser", "device", "office", "employee", "department", "date")
        }
        for event in events:
            ...
            name = " ".join(filter(None, [employee.get("firstName"), employee.get("lastName")])) or str(
                event.get("entityId") or "Unknown"
            )
        ...
        return {
            ...
            "metrics": {
                ...
                "employeesMigratedToSFace": migrated,
                "employeesMigratedToUniFace": migrated,
            },
            "breakdowns": {
                key: [{"label": label, "count": count} for label, count in value.most_common(100)]
